...
public class JobListView extends StackPane {
    ...
    private void refreshJobList(String searchText, String selectedDivisi) {
        ...
        List<Lowongan> filteredJobs = DatabaseManager.getAllLowongan().stream().filter(job -> {
            boolean matchText = searchText.isEmpty() || job.judul.toLowerCase().contains(searchText.toLowerCase())
    ...
    private void showJobDetailPage(Lowongan job) {
        ...
        Label descText = new Label(job.deskripsi);
        ...
        if (job.sisaSlot == 0 || isAccepted) {
            ...
            applyBtn.setOnAction(e -> {
                ...
                boolean namaLengkap = p.nama != null && !p.nama.trim().isEmpty();
                boolean emailLengkap = p.email != null && !p.email.trim().isEmpty();
                boolean teleponLengkap = p.telepon != null && !p.telepon.trim().isEmpty();
    ...
    private void showRegistrationModal(Lowongan job) {
        ...
        TextField name = new TextField(p.nama == null ? "" : p.nama);
        name.getStyleClass().add("form-input");
        name.setDisable(true);

        TextField email = new TextField(p.email == null ? "" : p.email);
        email.getStyleClass().add("form-input");
        email.setDisable(true);

        TextField phone = new TextField(p.telepon == null ? "" : p.telepon);
        phone.getStyleClass().add("form-input");
        phone.setDisable(true);

        TextArea about = new TextArea(p.tentangSaya == null ? "" : p.tentangSaya);
        about.setPromptText("Ceritakan tentang diri Anda...");
        about.setPrefRowCount(4);
        about.getStyleClass().add("form-input");

        Label cvLabelText = new Label((p.cvPath == null || p.cvPath.isEmpty()) ? "📄 Unggah CV (PDF)" : p.cvPath);
        ...
        Label mlLabelText = new Label(
                (p.motletPath == null || p.motletPath.isEmpty()) ? "📄 Unggah Motivation Letter (PDF)" : p.motletPath);
        ...
        submit.setOnAction(e -> {
            if (cvLabelText.getText().startsWith("📄") || mlLabelText.getText().startsWith("📄")) {
                showAlert("Error", "CV dan Motivation Letter harus diunggah!");
                return;
            }
            String newId = "LAM-" + System.currentTimeMillis();
            Lamaran newLamaran = new Lamaran(newId, job.id, p.nama == null ? "" : p.nama,
                    p.email == null ? "" : p.email,
                    p.telepon == null ? "" : p.telepon, about.getText(),
