...
public class AdminHomeView extends StackPane {
    ...
    private HBox createHeader() {
        ...
        iconBox.getChildren().addAll(bellIcon, miniLogo);
    ...
    private VBox createTotalLowonganDetail() {
        ...
        for (Lowongan l : DatabaseManager.getAllLowongan()) {
            String status = l.getSisaSlot() == -1 ? "Full / Unlimited" : "Tersisa " + l.getSisaSlot() + " Slot";
    ...
    private VBox createTotalPelamarDetail() {
        ...
        for (var lamaran : DatabaseManager.getAllLamaran()) {
            ...
            table.getChildren().add(createPelamarRow(String.valueOf(i++), lamaran.namaPelamar, divisi, false));
        }
        if (i == 1)
    ...
    private VBox createLowonganTersediaDetail() {
        ...
        for (Lowongan l : DatabaseManager.getAllLowongan()) {
            if (l.getSisaSlot() > 0)
                list.getChildren()
                        .add(createDetailCard(l.getJudul(), l.getDivisi(), "Tersisa " + l.getSisaSlot() + " Slot"));
        }
        if (list.getChildren().isEmpty())
            list.getChildren().add(new Label("Tidak ada lowongan tersedia."));
        ScrollPane scroll = new ScrollPane(list);
        scroll.setFitToWidth(true);
        scroll.setStyle(
                "-fx-background-color: transparent; -fx-background: transparent; -fx-border-color: transparent;");
    ...
    private HBox createDetailCard(String titleText, String divText, String statusText) {
        ...
        div.setTextFill(Color.web("#4A5568"));
    ...
    private HBox createPelamarRow(String no, String name, String div, boolean isHeader) {
        ...
        if (isHeader) {
            ...
            ((Label) nameBox.getChildren().get(0)).setTextFill(Color.web("#1A202C"));
