...
public class DatabaseManager {
    ...
    public static int getLowonganTersediaCount() {
        return (int) getAllLowongan().stream().filter(l -> l.getSisaSlot() > 0 || l.getSisaSlot() == -1).count();
    }
            ...
            if (lamaran != null) {
                ...
                if (newStatus.equals("Diterima") && !oldStatus.equals("Diterima")) {
                    try (PreparedStatement p = conn
                        ...
                        p.setString(1, lamaran.getEmailPelamar());
                        p.executeUpdate();
                    }
                    decreaseSlotTx(conn, lamaran.getIdLowongan());
                }
                if (oldStatus.equals("Diterima") && !newStatus.equals("Diterima")) {
                    ...
                    increaseSlotTx(conn, lamaran.getIdLowongan());
                }
            }
            conn.commit();
        } catch (SQLException e) {
    ...
    public static boolean isLowonganAvailable(String lowonganId) {
        Lowongan l = getLowonganById(lowonganId);
        return l != null && (l.getSisaSlot() > 0 || l.getSisaSlot() == -1);
    }
}