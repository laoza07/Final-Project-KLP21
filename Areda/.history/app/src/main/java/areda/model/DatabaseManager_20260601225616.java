...
public class DatabaseManager {
    ...
    // ✅ FIX UTAMA: Transaksi aman dengan try-with-resources, logika slot konsisten
    public static void updateLamaranStatus(String lamaranId, String newStatus) {
        try (Connection conn = DatabaseConnection.getConnection()) {
            ...
            // 3. Logika slot & accepted flag - HANYA jika status benar-benar berubah
            if (!oldStatus.equals(newStatus)) {
                if ("Diterima".equals(newStatus)) {
                    Lowongan l = getLowonganById(lowonganId);
                    // ✅ Hanya kurangi slot jika masih > 0
                    ...
                    // ✅ Tambah slot kembali jika dibatalkan dari Diterima
                    try (PreparedStatement p = conn
                            .prepareStatement("UPDATE lowongan SET sisa_slot = sisa_slot + 1 WHERE id = ?")) {
                        p.setString(1, lowonganId);
                        p.executeUpdate();
            ...
            // Rollback otomatis saat connection tertutup (try-with-resources)
        }
    }

    public static boolean isLowonganAvailable(String lowonganId) {
        Lowongan l = getLowonganById(lowonganId);
        // ✅ FIX: Hanya tersedia jika > 0 (tidak ada lagi -1)
