...
public class JobListView extends StackPane {
    ...
    private void showJobDetailPage(Lowongan job) {
        ...
        Label skillTitle = new Label("Skill yang dibutuhkan");
        skillTitle.setFont(Font.font("Arial", FontWeight.BOLD, 16));
        StringBuilder skillsText = new StringBuilder();
        for (String skill : job.skillDibutuhkan)
            skillsText.append("• ").append(skill).append("\n");
        Label skills = new Label(skillsText.toString());
        skillBox.getChildren().addAll(skillTitle, skills);
        ...
        if (job.sisaSlot == 0) {
            ...
            applyBtn.setOnAction(e -> {
                ...
                if (l == null) {
                    showAlert("Error", "Lowongan tidak ditemukan atau sudah dihapus!");
                    return;
                }

                if (l.sisaSlot == 0) {
                    showAlert("Error", "Maaf, lowongan ini sudah penuh!");
                    showJobListPage();
                    return;
                }

                Profil p = DatabaseManager.getProfil();
                if (DatabaseManager.hasUserApplied(job.id, p.email)) {
                    showAlert("Peringatan",
                            "Anda sudah melamar untuk posisi ini!\nAnda tidak dapat melamar dua kali untuk posisi yang sama.");
                    return;
                }
    ...
    private HBox createHeader(String titleText) {
        ...
        icons.getChildren().addAll(new Label(""), new Circle(15, Color.LIGHTGRAY));
